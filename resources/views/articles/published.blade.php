...
@section('breadcrumb', 'Published')

@section('content')
    @php
        // Dummy Data tailored for Published Articles
        $dummyPublished = [
            [
                'id' => 201,
            ...
            [
                ...
                'slug' => 'complete-guide-ui-ux-trends',
                'category' => 'Design',
                'author' => 'Jane Doe',
                'views' => '8.2k',
                'comments' => '156',
                'published_at' => 'Oct 18, 2026',
                'image' => 'https://images.unsplash.com/photo-1561070791-2526d30994b5?auto=format&fit=crop&w=150&h=100&q=80'
            ],
            [
                'id' => 203,
                'title' => 'Local Startup Raises $5M in Seed Funding',
                'slug' => 'local-startup-raises-5m',
                'category' => 'Business',
                'author' => 'Alex Smith',
                'views' => '5.1k',
                'comments' => '89',
                'published_at' => 'Oct 12, 2026',
                'image' => 'https://images.unsplash.com/photo-1556761175-5973dc0f32e7?auto=format&fit=crop&w=150&h=100&q=80'
            ],
        ];
    @endphp
    ...
    <div x-data="{
        ...
        <div class="mb-4 flex flex-col gap-4 sm:mb-6 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-xl font-bold text-foreground sm:text-2xl">Published Articles</h1>
                <p class="mt-1 text-sm text-muted-foreground">Manage and track the performance of your live content.</p>
            </div>
        ...
        {{-- Stats Cards (Tailored for Published Performance) --}}
        <div class="mb-4 grid grid-cols-2 gap-3 sm:mb-6 sm:gap-4 lg:grid-cols-4">
            ...
            <div class="rounded-xl border border-border bg-card p-4 sm:p-6 shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-medium text-muted-foreground sm:text-sm">Total Published</p>
            ...
            <div class="rounded-xl border border-border bg-card p-4 sm:p-6 shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-medium text-muted-foreground sm:text-sm">Total Views (30d)</p>
                        <p class="mt-1 text-lg font-bold text-foreground sm:text-2xl">1.2M</p>
                    </div>
            ...
            <div class="rounded-xl border border-border bg-card p-4 sm:p-6 shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-medium text-muted-foreground sm:text-sm">Comments</p>
                        <p class="mt-1 text-lg font-bold text-foreground sm:text-2xl">14.5k</p>
                    </div>
                    <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-info/10 sm:h-12 sm:w-12">
                        <i class="ph-bold ph-chats-circle text-lg text-info sm:text-xl"></i>
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-xl border border-border bg-card p-4 sm:p-6 shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-medium text-muted-foreground sm:text-sm">Avg. Read Rate</p>
                        <p class="mt-1 text-lg font-bold text-foreground sm:text-2xl">68%</p>
                    </div>
        ...
        <div class="mb-4 flex flex-col gap-3 sm:mb-6 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div class="flex items-center gap-2 w-full lg:max-w-md">
                ...
                <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-border bg-card">
                    <input type="checkbox" x-model="selectAll" @change="selected = selectAll ? [{{ implode(',', array_column($dummyPublished, 'id')) }}] : []"
                ...
                <div class="relative flex-1">
                    ...
                    <input type="text" placeholder="Search published articles..."
        ...
        <div class="space-y-3">
            @foreach($dummyPublished as $article)
                        ...
                        <div class="flex items-center justify-between lg:justify-end gap-4 w-full lg:w-auto mt-2 lg:mt-0 pt-4 lg:pt-0 border-t border-border lg:border-0">
                            ...
                            <div class="flex items-center gap-4 mr-2">
                                ...
                                <div class="hidden sm:flex flex-col items-end">
                                    <span class="text-xs text-muted-foreground flex items-center gap-1"><i class="ph-bold ph-chats-circle"></i> Comments</span>
                                    <span class="text-sm font-semibold text-foreground">{{ $article['comments'] }}</span>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-1 shrink-0">
                                <a href="#" target="_blank" class="rounded-lg p-2 text-muted-foreground transition-colors hover:bg-success/10 hover:text-success" title="View Live Article">
                                    <i class="ph-bold ph-arrow-square-out"></i>
                                </a>
                                <button class="rounded-lg p-2 text-muted-foreground transition-colors hover:bg-accent/10 hover:text-accent" title="Unpublish to Draft">
                                    <i class="ph-bold ph-archive-box"></i>
                                </button>
        ...
        <div class="mt-4 flex items-center justify-between rounded-xl border border-border bg-card px-4 py-3 sm:mt-6 sm:px-6 shadow-sm">
            <p class="text-xs text-muted-foreground sm:text-sm">
                Showing <span class="font-medium text-foreground">1</span> to <span class="font-medium text-foreground">3</span> of <span class="font-medium text-foreground">982</span> published articles
            </p>
            <div class="flex items-center gap-1">
                ...
                <button class="rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-foreground transition-colors hover:bg-muted">99</button>
